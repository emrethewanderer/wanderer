-- ═══════════════════════════════════════════════════════════════════════════
-- 045 — GÖZLEMEVİ · RİTÜELLERİN NABZI  (İç Çalışma 05 rev.3 · boşluk A)
--
-- Ritüel mimarisi ürünün dönüşüm motorudur ve motorun devri ölçülmüyordu:
-- dört direk (Hayal, Kendinle Konuşmak, değerlendirme, Engel Atlası) ile üç
-- yeni yüzey (Dinlenme, Derin Çalışma, Sefer) hiç yazmıyordu; Günlük Ritüel
-- ile Seri Mührü yalnız törenin SÜRESİNİ yazıyordu. "Kaç gezgin başladı,
-- kaçı sonuna kaldı, hangi direğe hiç dokunulmuyor" sorusu cevapsızdı.
--
-- Şema DEĞİŞMEZ: nabız mevcut usage_events satırlarıdır (kind='ritus',
-- 00f wtLogRitus). Bu dosyanın yaptığı tek şey admin_usage_report'a
-- 'ritus_pulse' bloğunu eklemek — yani fonksiyonu yeniden kurmak.
--
-- Gövde 044'ün üstüne biner; sıra 000 → 041 → 042 → 044 → 045 olduğu sürece
-- güncel tanım budur.
--
-- SÖZ-TUTMA ORANI burada tek meşru yerinden okunur: Günlük Ritüel'in
-- adim=2 satırı sözün VERİLDİĞİ an, adim=3 akşam hesabının verildiği an,
-- meta.n o hesapta TUTULAN söz sayısıdır. Oran uydurulmaz — payı da paydası
-- da kullanıcının kendi eylemidir (§6.10).
--
