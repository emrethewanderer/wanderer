-- ═══════════════════════════════════════════════════════════════════════════
-- 046 — GÖZLEMEVİ · EŞİĞİN NABZI  (İç Çalışma 06 rev.2 · boşluk A · K7)
--
-- Eşik, uygulamanın en pahalı dakikasıdır: kullanıcı burada ya kendi kartını
-- yazar ya da geri döner. Bu dakikanın hiçbir izi yoktu — perde kaç saniye
-- izlendi, hangi kategoride kalem düştü, kaç kişi "Bu Kart Benim" dedi, kimse
-- bilmiyordu. Bu dosya o dakikaya bir nabız takar; eşiğin kendisine dokunmaz,
-- yalnız ölçer.
--
-- Şema DEĞİŞMEZ: nabız mevcut usage_events satırlarıdır (kind='esik',
-- 00f wtLogEsik). Bu dosyanın yaptığı tek şey admin_usage_report'a
-- 'esik_pulse' bloğunu eklemek — yani fonksiyonu yeniden kurmak.
--
-- Gövde 045'in üstüne biner; sıra 000 → 041 → 042 → 044 → 045 → 046 olduğu
-- sürece güncel tanım budur. 045'in TÜM blokları (overview · screens ·
-- heatmap · transitions · trend · chat_depth · mode_pulse · memory_pulse ·
-- latency_pulse · ctx_pulse · kart_pulse · ritus_pulse · users ·
-- silent_users) aynen taşınır — bir blok düşerse Gözlemevi'nin o kartı
-- kaybolur.
--
-- DENETİM DÜZELTMESİ (046, taşıma sırasında bulundu): 045'in memory_pulse ve
-- latency_pulse blokları `duration_ms` sütununa çıplak referans veriyordu;
-- ana CTE (`ev`) bu sütunu hiç dışa açmıyor, yalnız kırpılmış `dur_ms`'i
-- açıyor (aşağıdaki `LEAST(GREATEST(duration_ms, 0), 1800000) AS dur_ms`
-- satırı). PL/pgSQL gövdesindeki SQL ifadeleri CREATE anında yalnız
-- SÖZDİZİMİ olarak denetlenir; sütun çözümlemesi ilk çalıştırmaya kalır —
-- yani CREATE "başarılı" der, fonksiyon ÇAĞRILDIĞINDA "column ev.duration_ms
-- does not exist" ile patlar. Sessiz olmasının sebebi bu: migration'ı koşan
-- yeşil çıktı görür, kırığı ancak Gözlemevi'ni açan bulur — ve o an tek bir
-- blok değil, panelin TAMAMI kaybolur (RPC hiç dönmez). Hata 042'den beri (044, 045) taşınmış; bu dosyada iki blokta
-- da `dur_ms`'e düzeltildi (davranış aynı: aynı kırpılmış süre, üstelik artık
-- 30 dakikalık absürt-değer tavanına da tabi). 042/044/045 dosyalarına
-- dokunulmadı — onlar tarihsel kayıt, düzeltme yalnız güncel tanımda.
--
-- Yeni eklenen 'esik_pulse' bloğu eşiği bir huni + dört kadran olarak okur:
-- doğuş oranı, kalemin düştüğü kategori, perdenin bedeli, sentez sağlığı.
-- Oranın payı da paydası da HAM SAYI olarak döner — oranı panel (13q) kurar,
-- payda 0 ise gösterilmez (§6.10, kanıtsız değer yok).
--
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'memory_pulse', (SELECT jsonb_build_object(
        ...
        'avg_ms',   COALESCE(ROUND(AVG(dur_ms) FILTER (WHERE dur_ms > 0))::INT, 0),  -- düzeltme: ev'de 'duration_ms' yok, kırpılmış dur_ms kullanılır
    ...
    'latency_pulse', (SELECT jsonb_build_object(
        'total_turns', COUNT(*),
        'p50_ms', COALESCE(ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY dur_ms))::INT, 0),   -- düzeltme: dur_ms
        'p95_ms', COALESCE(ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY dur_ms))::INT, 0),  -- düzeltme: dur_ms
