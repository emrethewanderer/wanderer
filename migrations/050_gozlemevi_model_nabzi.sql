-- ═══════════════════════════════════════════════════════════════════════════
-- 050 — GÖZLEMEVİ · ÜÇ SESİN NABZI  (İç Çalışma 08 rev.2 · boşluk A)
--
-- Ürün üç ses olduğunu söylüyor: Öz (bireysel) · Bağ (ilişki) · Eser (iş).
-- Bu bir özellik değil, kimlik iddiasıdır — ve 18 Temmuz'dan beri kanıtsızdı.
-- On bir kanallı kadranın hiçbirinde `oz`/`bag`/`eser` geçmiyordu.
--
-- Şema DEĞİŞMEZ: nabız mevcut usage_events satırlarıdır (kind='model',
-- 00f wtLogModel). Bu dosyanın yaptığı tek şey admin_usage_report'a
-- 'model_pulse' bloğunu eklemek — yani fonksiyonu yeniden kurmak.
--
-- Gövde 049'un üstüne biner; sıra 000 → 041 → … → 049 → 050 olduğu sürece
-- güncel tanım budur. 049'un TÜM blokları aynen taşınır — bir blok düşerse
-- Gözlemevi'nin o kartı kaybolur.
--
-- SEÇİM İLE KİLİT AYRI SAYILIR: 10w:111 Free katmanını Öz'e kilitler.
-- İkisini tek dağılımda toplamak "herkes Öz'ü seviyor" diye okunur; oysa
-- ölçülen şey mahkûmiyettir. 'sec' niyet, 'kilit' karşılanmamış talep,
-- 'dus' sessiz kayıptır.
--
-- TUR SAYISI YENİ BİR OLAYDAN GELMEZ: kind='latency' satırlarının meta.fm
-- alanından okunur (İç Çalışma 08 rev.2 · K2). Aynı turu iki kanaldan
-- saymak kadranda iki farklı rakam doğurur. Bilinçli sınır: latency satırı
-- yalnız ilk görünür harf geldiğinde yazılır — bu sayı BAŞARILI turlardır.
--
