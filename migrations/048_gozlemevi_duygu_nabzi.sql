-- ═══════════════════════════════════════════════════════════════════════════
-- 048 — GÖZLEMEVİ · YANILMA NABZI  (13D §10 · K13 · FAZ 15)
--
-- Duygu Motoru artık kendi hata oranını ölçüyor: her yüzey için kaç kez
-- konuştu, kaç kez düzeltildi ("beni yanlış okudun"). Kullanıcı başına
-- gerçek KAPANMA kararı (dgYanilmaKapali) 13D'nin kendisinde, kayan
-- pencere 12 üzerinden, SafeStorage'daki İklim'de verilir — bu dosya o
-- kararı yeniden hesaplamaz. Yaptığı tek şey admin_usage_report'a
-- 'duygu_pulse' bloğunu eklemek: dönem boyunca TÜM gezginlerin toplam
-- konuşma/düzeltme sayısı, Gözlemevi'nin (13q) göreceği aynadır — bir
-- yüzeyin burada yüksek oranla görünmesi "herkes için kapandı" demek
-- değil, "bu yüzeyde düzeltme sık" demektir.
--
-- Şema DEĞİŞMEZ: nabız mevcut usage_events satırlarıdır (kind='duygu',
-- 00f wtLogDuygu — FAZ 12'den beri var, FAZ 15'te meta'ya `yuzey` ve
-- `duzeltildi` eklendi). Bu dosyanın yaptığı tek şey admin_usage_report'u
-- yeniden kurmak.
--
-- Gövde 046'nın üstüne biner; sıra 000 → 041 → … → 046 → 047 → 048 olduğu
-- sürece güncel tanım budur. 046'nın TÜM blokları (overview · screens ·
-- heatmap · transitions · trend · chat_depth · mode_pulse · memory_pulse ·
-- latency_pulse · ctx_pulse · kart_pulse · ritus_pulse · esik_pulse ·
-- users · silent_users) aynen taşınır — bir blok düşerse Gözlemevi'nin o
-- kartı kaybolur (047 şemaya dokunmadı, admin_usage_report'u değiştirmedi).
--
-- YUZEY GERİYE DÖNÜK UYUM: meta.yuzey FAZ 15'ten ÖNCE yazılan satırlarda
-- hiç yoktur (NULL). O eski satırlar tek çağıranın (sohbet) izidir — 'sohbet'
-- varsayılanına düşürülür, veri kaybolmaz.
--
-- ORAN BURADA HESAPLANMAZ: konustu/duzeltildi HAM SAYI olarak döner —
-- oranı panel (13q) kurar, konustu payda 5'in altındaysa gösterilmez
-- (§6.10, kanıtsız değer yok — 13D DG_YANILMA_MIN_N'in görünüm ikizi).
--
